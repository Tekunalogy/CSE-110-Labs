...
class JournalEntry extends HTMLElement {
  ...
  set entry(entry) {
    ...
    // CODE GOES HERE
    let sh = this.shadowRoot.children[1];

    sh.getElementsByClassName("entry-title")[0].innerHTML = entry.title;
    sh.getElementsByClassName("entry-date")[0].innerHTML = entry.date;    
    sh.getElementsByClassName("entry-content")[0].innerHTML = entry.content;
